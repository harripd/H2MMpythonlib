...
def model_almost_equal(model1:hm.h2mm_model, model2:hm.h2mm_model):
    """
...
def limits_func_system_converged(new, current, old):
    """Test limits only function, no args/kwargs, return only model"""
    new.prior = np.ones(new.prior.size)/new.prior.size
    return new


def limits_func_user_converged(new, current, old, bound):
    """Test limits/converged function with bounds, return model, code"""
...
def limits_func_bad_size(new, current, old):
    """Test bad limits function, used to confirm proper dealocation after bad limits function"""
    return hm.factory_h2mm_model(current.nstate+1, current.ndet+1)
...
def print_func_str(new, current, old, titer, ttotal, *args):
    """Test string returning print function"""
    return str(new) + "\n"


def print_func_num(new, current, old, titer, ttotal, *args):
    """Print func returns number"""
    return 32


def print_func_exception(new, current, old, titer, ttotal, *args):
    """print func raises exception, used to test handling of error in function"""
...
def data_gen_np():
    """produce smaple data as numpy object array"""
    dets, times, nphot = data_gen_list()
...
def test_MakeModel():
    ...
    model = hm.h2mm_model(prior,trans,obs)
...
def test_Factory():
    ...
    """
    model = hm.factory_h2mm_model(3,2)
    assert type(model) == hm.h2mm_model
    assert model.nstate == 3
    assert model.ndet == 2
    assert model.k == 11
...
def test_Model_access():
    ...
    """
    model = hm.factory_h2mm_model(4,3)
...
def test_InputType(simple_data_all):
    ...
    out = hm.EM_H2MM_C(hm.factory_h2mm_model(2,2), dets, times)
    assert isinstance(out, hm.h2mm_model)
...
def opt_model(request):
    """Model optimized against data, gives 2, 3, and 4 states"""
...
def get_output_model(arr)->hm.h2mm_model:
    for model in reversed(arr):
        if model.conv_code & hm.convcode.output:
            return model
    raise ValueError("no output model in arr")


def model_opt():
    """Iterate arrays of optimizations"""
    dets, times, nphot = data_gen_list()
    out1 = hm.EM_H2MM_C(hm.factory_h2mm_model(2,2), dets, times, max_iter=10, opt_array=True, num_cores=1)
    out2 = hm.EM_H2MM_C(hm.factory_h2mm_model(3,2), dets, times, max_iter=10, opt_array=True, num_cores=1)
    out3 = np.concatenate([out1, out2])
    # find optimized model
    yield get_output_model(out1)
    yield out2
    yield out3
...
def test_H2MM_arr(model_array,simple_data_all):
    ...
    """
    dets, times, nphot = simple_data_all
    arr_out = hm.H2MM_arr(model_array, dets, times, num_cores=1)
    assert isinstance(model_array,  hm.h2mm_model) == isinstance(arr_out, hm.h2mm_model), 'ouput of different type han input'
    arr_out = np.array([arr_out,], dtype=np.object_) if isinstance(arr_out, hm.h2mm_model) else arr_out
    model_array = np.array([model_array,], dtype=np.object_) if isinstance(model_array, hm.h2mm_model) else model_array
...
def test_Viterbi(opt_model,simple_data_all):
    ...
    """
    dets, times, nphot = simple_data_all
    path, scale, ll, icl = hm.viterbi_path(opt_model, dets, times, num_cores=1)
    ...
    for BIC, LL, log, logpath in product(*((False, True) for _ in  range(4))):
        if sum((BIC, LL, log, logpath)) == 0:
            continue
        out = hm.path_loglik(opt_model, dets, times, path, BIC=BIC, total_loglik=LL, loglikarray=log, loglikpath=logpath)
...
def test_Viterbi_multi_core(opt_model,simple_data_all):
    ...
    """
    dets, times, nphot = simple_data_all
    path, scale, ll, icl = hm.viterbi_path(opt_model, dets, times, num_cores=3)
    path1, scale1, ll1, icl1 = hm.viterbi_path(opt_model, dets, times, num_cores=1)
    ...
    for BIC, LL, log in combinations_with_replacement((True, False), 3):
        if sum((BIC, LL, log)) == 0:
            continue
        out = hm.path_loglik(opt_model, dets, times, path, BIC=BIC, total_loglik=LL, loglikarray=log)
        if sum((BIC, LL, log)) == 1:
            out = (out, )
        cur_pos = 0
        if BIC:
...
def test_Limits(simple_data_list):
    ...
    limits = hm.h2mm_limits(min_trans=1e-7, max_trans=1e-1)
...
def test_Limits_usr(opt_model, simple_data_list):
    ...
    """
    dets, times, nphot = simple_data_list
    out = hm.EM_H2MM_C(opt_model, dets, times, bounds_func=limits_func_system_converged)
    assert isinstance(out, hm.h2mm_model), "EM_H2MM_C returned wrong type"
    out = hm.EM_H2MM_C(opt_model, dets, times, bounds_func=limits_func_user_converged, bounds=10)
    assert isinstance(out, hm.h2mm_model)
    assert out.niter == 10
...
def test_User_print_str(opt_model, simple_data_list):
    ...
    """
    dets, times, nphot = simple_data_list
    hm.EM_H2MM_C(opt_model, dets, times, print_func=print_func_str)
    hm.EM_H2MM_C(opt_model, dets, times, print_func=print_func_num)
    with pytest.raises(Exception):
        hm.EM_H2MM_C(opt_model, dets, times, print_func=print_func_exception)
...
def test_Sim():
    ...
    model_init = hm.h2mm_model(prior_init,trans_init,obs_init)
    sim_traj = [hm.sim_phtraj_from_times(model_init,tm) for tm in times]
    paths, colors = [], []
...
def test_BadData(opt_model):
    ...
    with pytest.raises(ValueError):
        hm.EM_H2MM_C(opt_model, dets, time)
    dets = data_gen_list()[0].copy()
    dets[0][0] = 2
    with pytest.raises(Exception):
        hm.EM_H2MM_C(opt_model, dets, data_gen_list()[1])
    dets[0] = np.concatenate([dets[0], [1]])
    with pytest.raises(ValueError):
        hm.EM_H2MM_C(opt_model, dets, time)
    dets, time = data_gen_list()[0].copy(), data_gen_list()[1].copy()
    with pytest.raises(Exception):
        hm.EM_H2MM_C(opt_model, time, dets)
...
def test_BadLimits(opt_model, simple_data_list):
    ...
    dets, times, nphot = simple_data_list
    with pytest.raises(ValueError):
        hm.EM_H2MM_C(opt_model, dets, times, bounds_func=limits_func_bad_size)
    with pytest.raises(TypeError):
        hm.EM_H2MM_C(opt_model, dets, times, bounds_func=limits_func_bad_return)
...
def test_OOp(simple_data_all):
    ...
    """
    dets, times, nphot = simple_data_all
    in_mod = hm.factory_h2mm_model(3,2)
...
def test_hash():
    """Test hashing function and ==/!= for h2mm_model"""
    m1 = hm.h2mm_model(np.array([0.2,0.5,0.3]), 
    ...
    hash(mh1)
    assert mh1 == m1
    m2 = hm.factory_h2mm_model(3,2).sort_states()
    assert m2 != mh1


def verify_allmodel(model, nstate, ndet, nphot, niter, conv, ll, prior, trans, obs):
    """Check function, model matches inputs"""
...
def test_immutable(simple_data_np):
    """Verify model set to immutable stays immutable, and properly sorted"""
    dets, times, nphot = simple_data_np
    m1 = hm.factory_h2mm_model(2, 3).sort_states()
    m1nstate = m1.nstate
    m1ndet = m1.ndet
    m1nphot = m1.nphot
    m1niter = m1.niter
    ...
    verify_allmodel(m1, m1nstate, m1ndet, m1nphot, m1niter, m1conv, m1ll, m1prior, m1trans, m1obs)
    m2 = hm.EM_H2MM_C(m1, dets, times)
    verify_allmodel(m1, m1nstate, m1ndet, m1nphot, m1niter, m1conv, m1ll, m1prior, m1trans, m1obs)
    m3 = m1.optimize(dets, times, inplace = False)
    verify_allmodel(m1, m1nstate, m1ndet, m1nphot, m1niter, m1conv, m1ll, m1prior, m1trans, m1obs)
    with pytest.raises(TypeError):
        m1.optimize(dets, times, inplace=True)
    hm.H2MM_arr([m1, m2, m3], dets, times)
    verify_allmodel(m1, m1nstate, m1ndet, m1nphot, m1niter, m1conv, m1ll, m1prior, m1trans, m1obs)
    path, _, _ , _ = hm.viterbi_path(m1, dets, times)
    verify_allmodel(m1, m1nstate, m1ndet, m1nphot, m1niter, m1conv, m1ll, m1prior, m1trans, m1obs)
    hm.path_loglik(m1, dets, times, path)
    verify_allmodel(m1, m1nstate, m1ndet, m1nphot, m1niter, m1conv, m1ll, m1prior, m1trans, m1obs)
...
def test_bytes(opt_model):
    ...
    bmodel = hm.h2mm_model.frombytes(bt)
...
def test_EM_H2MM_C_ll_gamma(simple_data_tuple):
    ...
    for ll, gamma, opt_array in product(*([False, True] for _ in range(3))):
        out = hm.EM_H2MM_C(hm.factory_h2mm_model(2,3), indexes, times, ll=ll, gamma=gamma, opt_array=opt_array)
...
def test_H2MM_arr_ll_gamma(simple_data_tuple, model_array):
    ...
    for ll, gamma in product([False, True], [False, True]):
        if gamma and not isinstance(model_array, hm.h2mm_model):
            continue
        out = hm.H2MM_arr(model_array, indexes, times, gamma=gamma, ll=ll)
        if not ll and not gamma:
            out = (out, )
        model_t, out = out[0], out[1:]
        if ll:
            ll_array_t, out = out[0], out[1:]
            if isinstance(model_t, hm.h2mm_model):
                assert np.allclose(model_t.loglik, np.sum(ll_array_t)), "Sum of loglik different from model loglik"
            else:
                assert np.allclose([m.loglik for m in model_t], np.sum(ll_array_t, axis=-1)), "Sum of loglik different from model loglik"
        if gamma:
            gammaarray_t, out = out[0], out[1:]
        assert not out, "Incorrect number of returned values"
        if isinstance(model_t, hm.h2mm_model):
            model_t = np.array([model_t, ], dtype=object)
        if model is None:
            model = model_t
        else:
            for m, m_t in zip(model.ravel(), model_t.ravel()):
                assert model_almost_equal(m, m_t), "Different optimization results returned"
        if ll and ll_array is None:
            ll_array = ll_array_t
        elif ll:
...
def test_evaluate_ll_gamma(simple_data_tuple):
    ...
    for ll, gamma in product([False, True], [False, True]):
        if gamma and not isinstance(model_array, hm.h2mm_model):
            continue
        out =  hm.factory_h2mm_model(2, 3).evaluate(indexes, times, gamma=gamma, ll=ll)
        if not ll and not gamma:
            out = (out, )
        model_t, out = out[0], out[1:]
        if ll:
