// File: h2mm_squarem_gamma.c
// Author: Paul David Harris
// Purpose: Wrapper function for command line interface to test h2mm_squarem_gamma
...
int main(int argc, char **argv)
{
	...
	for (k = 0; k < argc - 2; k++){
		...
		h2mm_squarem_ll_gamma(num_burst, len_bursts, times, detectors, &models[k], out_mod, llarr, &gamma, limits, lim_func[0], minmaxlimit, &baseprint, NULL);
		printf("Done h2mm_optimize\n");
		//for (i = 0; i < num_burst; i++){
		for (i = 0; (i < 2) || (i < num_burst); i++){
			printf("[");
			for (j = 0; (j < len_bursts[i]*out_mod->nstate) || (j < 4); j++){
				printf("%f, ", gamma[i][j]);
				gamma[i][j] = 0.0;
			}
			printf("]\n");
		...
		double llsum = 0.0;
		for ( i = 0; i < num_burst; i++) llsum += llarr[i];
		printf("ll = [%f, %f], sum = %f, diff = %f\n", llarr[0], llarr[num_burst-1], llsum, llsum - out_mod->loglik);
		free(gamma);
