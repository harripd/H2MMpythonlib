...
int main(int argc, char **argv)
{
	...
	h2mm_minmax *minmaxlimit = (h2mm_minmax*) calloc(1,sizeof(h2mm_minmax));
	double* llarr = (double*) calloc(num_burst, sizeof(double));
	for (k = 0; k < argc - 2; k++)
	{
		...
		// *************************************************************
		//h2mm_optimize_ll(num_burst, len_bursts, times, detectors, &models[k], out_mod, llarr, limits, lim_func[0], minmaxlimit, &baseprint, NULL);
		h2mm_squarem_ll(num_burst, len_bursts, times, detectors, &models[k], out_mod, llarr, limits, lim_func[0], minmaxlimit, &baseprint, NULL);
		// *************************************************************
		printf("Done h2mm_optimize\n");
		print_model(out_mod);
		printf("loglik: \n");
		for ( i = 0; i <= out_mod->niter; i++) printf("%f, ", llarr[i]);
		printf("\n");
		out_mod->conv = 0;
		zero_model(out_mod);
