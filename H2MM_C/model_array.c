...
int calc_multi_ll(int64_t num_burst, int64_t *burst_sizes, int32_t **burst_deltas, uint8_t **burst_det, int64_t num_models, h2mm_mod *models, double **llarr, lm *limits)
{
	...
	for (i = 0; i < num_models; i++)
	{
		...
		for(j = 0; j < limits->num_cores; j++) 
		{
			pthread_create(&tid[j],NULL, fwd_ll_only,(void*) &burst_submit[j]); // create a thread for each burst
			//~ printf("pthread[%ld] res=%d\n", j, res);
		}
		for(j = 0; j < limits->num_cores; j++) 
		{
...
#elif _WIN32
		for (j = 0; j < limits->num_cores; j++)
			tid[j] = CreateThread(NULL, 0, fwd_ll_only, (LPVOID)&burst_submit[j], 0, (LPDWORD)&windowsThreadId);
		WaitForMultipleObjects((DWORD)limits->num_cores, tid, TRUE, INFINITE); // Wait for all of the threads to finish
		for (j = 0; j < limits->num_cores; j++)
		{
			if (tid[j] != 0)
...
int calc_multi_ll_gamma(int64_t num_burst, int64_t *burst_sizes, int32_t **burst_deltas, uint8_t **burst_det, int64_t num_models, h2mm_mod *models, double **llarr, double ****gamma, lm *limits)
{
	...
	for (i = 0; i < num_models; i++)
	{
		...
		for(j = 0; j < limits->num_cores; j++) 
		{
			pthread_create(&tid[j], NULL, fwd_bck_ll_gamma,(void*) &burst_submit[j]); // create a thread for each burst
		}
		for(j = 0; j < limits->num_cores; j++) 
		{
			pthread_join(tid[j],NULL); // wait for all bursts to finish
...
#elif _WIN32
		for (j = 0; j < limits->num_cores; j++)
			tid[j] = CreateThread(NULL, 0, fwd_bck_ll_gamma, (LPVOID)&burst_submit[j], 0, (LPDWORD)&windowsThreadId);
		WaitForMultipleObjects((DWORD)limits->num_cores, tid, TRUE, INFINITE); // Wait for all of the threads to finish
		for (j = 0; j < limits->num_cores; j++)
		{
			if (tid[j] != 0)
