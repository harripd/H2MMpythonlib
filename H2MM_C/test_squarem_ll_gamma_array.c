...
int main(int argc, char **argv)
{
	...
	llarr = (double*) malloc(num_burst*sizeof(double));
	...
	for (k = 0; k < argc - 2; k++)
	{
		...
		// *************************************************************
		//h2mm_optimize_ll_array(num_burst, len_bursts, times, detectors, &models[k], &out_mod, limits, lim_func[0], minmaxlimit, &baseprint, NULL);
		h2mm_squarem_ll_gamma_array(num_burst, len_bursts, times, detectors, &models[k], &out_mod, llarr, &gamma, limits, lim_func[0], minmaxlimit, &baseprint, NULL);
		// *************************************************************
		printf("Done h2mm_optimize\n");
		printf("first model\n");
		print_model(&out_mod[0]);
		printf("optimized model\n");
		for (i = 0; i < limits->max_iter+2; i++){
			if (out_mod[i].conv & CONVCODE_OUTPUT){
				printf("found converged model at %ld\n", i);
				print_model(&out_mod[i]);
				break;
			}
			else if (out_mod[i].conv == 0) {
				printf("no optimal model found before empty array\n");
				break;
			}
		}
		for (j = 0; j < limits->max_iter+2; j++)
		{
			if (out_mod[j].obs != NULL){
				printf("model # %ld\n", j);
				if (out_mod[j].conv != 0) print_model(&out_mod[j]);
				else printf("Apparently non-calculated model\n");
				//printf("conv[%ld] = %ld, ll=%f\n", j, out_mod[j].conv, out_mod[j].loglik);
			}
		}
		printf("gamma[0] = [");
		for (j = 0; j < len_bursts[0]*models[k].nstate; j ++){
			printf("%f, ", gamma[0][j]);
			gamma[0][j] = 0.0;
		}
		printf("]\n");
		printf("gamma[-1] = [");
		for (j = 0; j < len_bursts[num_burst-1]*models[k].nstate; j ++){
			printf("%f, ", gamma[num_burst-1][j]);
			gamma[num_burst-1][j] = 0.0;
		}
		printf("]\n");
		double llsum = 0.0;
		for (j = 0; j < num_burst; j++) llsum += llarr[j];
		printf("loglik arr: [%f, ..., %f], sum = %f\n", llarr[0], llarr[num_burst-1], llsum);
		printf("free current model array\n");
		for (i=0; i < num_burst; i++)
		{
